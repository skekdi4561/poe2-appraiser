...
**"Windows 의 PC 보호" 경고가 뜨는 이유**: 코드 서명 인증서가 없는 개인 제작 프로그램이라
SmartScreen 이 처음 보는 파일로 취급합니다. `추가 정보 → 실행` 으로 진행하면 됩니다.
...
3. 공격 무기를 가격 체크하면 창 아래에 **시장 곡선 판정**이 붙습니다.
...
- **F7 위젯이 `Esc` 로 바로 안 닫힐 때가 있습니다.** 그럴 땐 `F7` 을 한 번 더 누르면 닫힙니다.
  (위젯이 포커스를 받은 적이 있어야 `Esc` 가 먹는 구조라 간헐적으로 어긋납니다 — 다음 버전에서 손봅니다.)
...
- 카카오 서버의 공격 무기 6종 검색에서만 보내며, 수집기가 현재 리그 표본만 곡선에 씁니다
